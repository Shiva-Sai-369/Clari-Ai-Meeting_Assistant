...
const Demo = () => {
  ...
  const captions = [
    { speaker: "Sarah", text: "Let's review the Q1 roadmap priorities.", time: "0:00" },
    { speaker: "John", text: "I'll take ownership of the mobile redesign project.", time: "0:15" },
    { speaker: "Sarah", text: "Great! We need mockups by Friday.", time: "0:22" },
  ];
  ...
  const summary = {
    ...
    actionItems: [
      ...
      { task: "Schedule follow-up review", owner: "Sarah", deadline: "Next Monday" },
    ],
  };
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-16">
            <div className="inline-block mb-6 px-4 py-2 rounded-full glass-card">
              <span className="text-sm font-medium gradient-text">Demo powered by GPT-4o Realtime API</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-bold mb-6">
              See <span className="gradient-text">EchoMeet</span> in Action
            </h1>
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
              Experience real-time transcription, AI summaries, and action item extraction
            </p>
          </div>
                    ...
                    {captions.map((caption, index) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className="text-xs text-muted-foreground">{caption.time}</span>
                        </div>
            ...
            <div className="space-y-6">
              {isPlaying && (
                <>
                  ...
                  <div className="glass-card p-8 rounded-2xl animate-fade-in" style={{ animationDelay: "0.3s" }}>
                    <div className="flex items-center gap-2 mb-6">
                    ...
                    <div className="space-y-4">
                      {summary.actionItems.map((item, index) => (
                        <div key={index} className="glass-card p-4 rounded-xl border-l-2 border-primary">
                          <p className="font-medium mb-2">{item.task}</p>
                          <div className="flex items-center gap-4 text-sm text-muted-foreground">
                            <span>Owner: <span className="text-foreground">{item.owner}</span></span>
                            <span>Due: <span className="text-foreground">{item.deadline}</span></span>
                          </div>
                        </div>
                      ))}
                    </div>
